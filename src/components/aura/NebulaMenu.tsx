    ...
    <>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent
          align="end"
          sideOffset={8}
          className="w-[340px] sm:w-[400px] p-0 bg-background/70 backdrop-blur-xl border-white/10 shadow-2xl rounded-2xl overflow-hidden"
        >
          {/* Sticky header */}
          <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-xl border-b border-white/5 px-4 py-3">
            <div className="flex items-center gap-2 mb-2">
              <Sparkles className="h-3.5 w-3.5 text-primary" />
              <h3 className="text-sm font-extrabold tracking-tight">Your iBloov Orbit</h3>
            </div>
            ...
            <>
              <DndContext collisionDetection={closestCenter} onDragEnd={onDragEnd}>
                <SortableContext items={orderedProducts.filter((p) => !p.alwaysLinked).map((p) => p.id)} strategy={rectSortingStrategy}>
                  <div className="grid grid-cols-3 xl:grid-cols-4 gap-2.5 p-4">
