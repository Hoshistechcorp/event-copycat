  ...
  return (
    ...
    >
      <button
        ...
        className={`relative w-full aspect-square rounded-2xl bg-card/60 backdrop-blur-sm border-2 border-white/10 flex flex-col items-center justify-center gap-3 p-4 transition-all duration-200 hover:-translate-y-1 hover:bg-card hover:border-white/20 ${linked ? `hover:${product.glow}` : "grayscale-[60%] opacity-70 hover:grayscale-0 hover:opacity-100"}`}
      >
        <Icon className={`h-10 w-10 ${product.color}`} strokeWidth={2.25} />
        <span className="text-sm font-extrabold text-foreground text-center leading-tight tracking-tight">{product.name}</span>

        {!linked && !product.alwaysLinked && (
          <span className="absolute top-2 right-2 inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full bg-primary text-primary-foreground text-[9px] font-bold animate-pulse">
            <Sparkles className="h-2.5 w-2.5" /> Set Up
          </span>
        )}
      </button>
